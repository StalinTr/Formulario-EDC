...
{\large$\mathbf{T} = \dfrac{\epsilon_0 \vec{E}^2+\frac{1}{\mu_0}\vec{B}^2}{2}\mathbf{\mathbb{I}}-(\epsilon_0 \vec{E}\circ \vec{E}+\frac{1}{\mu_0}\vec{B}\circ\vec{B})$}\\

{\large$  \mathrm{T}_{ij}^{M}=\mathrm{\varepsilon}_{0}c^2\left[\frac{E_iE_j}{c^2}+B_iB_j-\frac{1}{2}\delta_{ij}\left( \frac{E^2}{c^2}+B^2 \right) \right]$}
...
$ \vec{x}' = \mathbf{\Lambda}\vec{x} \;\;\;\;\Longrightarrow\;\;\;\; x'^i = \Lambda_{j}^{i} x^{j}$

$
    \mathbf{\Lambda} = \begin{pmatrix}
         \begin{array}{c|c}
             \gamma & -\gamma\vec{\beta}\\[0.5em] \hline\\[-0.5em]
             -\gamma\vec{\beta} & \mathbf{\mathbb{I}}+\dfrac{\gamma-1}{\beta^2}\vec{\beta}\circ\vec{\beta}
         \end{array}
         \end{pmatrix}=$\\
...
$\delta_{j}^{i} = g^{ik}g_{kj}= \mathbb{I}_{ij}\neq\delta_{ij}=g_{ik}\delta_{j}^{k}$\\
...
$\\
$
    \dot{\vec{\beta}} = \frac{1}{\gamma m c} \left[  \vec{F} - (\vec{F}.\vec{\beta})\vec{\beta}  \right] \qquad \frac{d\gamma}{dt'}= \gamma^3 (\vec{\beta}.\dot{\vec{\beta}})\\
    \vec{F} =\frac{d\vec{p}}{dt'}= \gamma m c \left[  \dot{\vec{\beta}} - \gamma^2 (\vec{\beta}.\dot{\vec{\beta}})\vec{\beta}  \right]
$\\
